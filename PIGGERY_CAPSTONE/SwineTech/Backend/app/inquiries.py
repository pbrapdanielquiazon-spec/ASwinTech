...
def respond_inquiry(inquiry_id: int, data: InquiryRespond, db: Session = Depends(get_db),
                    staff: models.User = Depends(require_roles(Role.ADMIN, Role.SALES))):
    obj = db.get(models.Inquiry, inquiry_id)
    if not obj:
        raise HTTPException(404, "Inquiry not found")
    obj.status = data.status
    obj.responded_by = staff.user_id
    obj.responded_at = datetime.utcnow()if data.status == InquiryStatus.RESPONDED else obj.responded_at
    db.add(obj); db.commit(); db.refresh(obj)
    return obj