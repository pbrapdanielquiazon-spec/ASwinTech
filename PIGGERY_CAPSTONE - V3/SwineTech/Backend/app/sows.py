...
from .models import AuditEntity, AuditAction, User
...
def create_sow_ep(
    ...
    log_audit(
        db,
        entity=AuditEntity.SOW,
        entity_id=sow.sow_id,
        action=AuditAction.CREATE,
        user_id=current.user_id,
        details={"payload": payload.model_dump(exclude_none=True, mode="json")}
    )
...
def update_sow_ep(
    ...
    # --- BEFORE SNAPSHOT for change summary ---
    before = {
        "sow_identifier": sow.sow_identifier,
        "status": sow.status,
        "mating_date": sow.mating_date,
    ...
    # Apply update (this commits & refreshes)
    ...
    # Build minimal change set for audit details
    changes = {
    ...
    log_audit(
        ...
        user_id=current.user_id,
        details={"changes": changes or {"note": "no effective field changes"}}
    )
    db.commit()
