...
CREATE TABLE supplies (
    ...
    category ENUM('feed', 'medicine', 'vaccine', 'tool', 'other') NOT NULL,
...
CREATE TABLE expenses (
    ...
    category ENUM('feed', 'medicine', 'vaccine', 'tool', 'other'),
...
========== ADD THIS AFTER THE TOP CODE ========== 
step 6
-- add the column (nullable first so migration won’t fail on old rows)
ALTER TABLE pig_health_records
  ADD COLUMN treatment_supply_id INT NULL;

-- add the foreign key
ALTER TABLE pig_health_records
  ADD CONSTRAINT fk_health_treatment_supply
  FOREIGN KEY (`treatment_supply_id`) REFERENCES `supplies` (`supply_id`)
  ON UPDATE CASCADE
  ON DELETE RESTRICT;
...
step 7
ALTER TABLE pig_health_records
  MODIFY COLUMN treatment_supply_id INT NOT NULL;
...
CREATE TABLE IF NOT EXISTS audit_events (
  ...
  entity_type  ENUM(
                  'PIG','LITTER','HEALTH','SOW','FEED_LOG','EXPENSE','SUPPLY',
                  'SALE'
               ) NOT NULL,
  ...
  INDEX idx_audit_recorded_by (recorded_by),
  INDEX idx_audit_entity      (entity_type, entity_id),

  CONSTRAINT fk_audit_recorded_by
...
install python 3.12 and check add to PATH when installing (ignore if mana)
