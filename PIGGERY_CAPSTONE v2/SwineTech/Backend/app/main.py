...
api.include_router(auth_router)
api.include_router(list_users.router, prefix="/api")
...
def update_feeding_log(log_id: int, data: FeedingLogUpdate, db: Session = Depends(get_db),
                       current_user: models.User = Depends(get_current_user),
):
    obj = _feedinglog_or_404(db, log_id)

    # if litter_id is changing, validate
