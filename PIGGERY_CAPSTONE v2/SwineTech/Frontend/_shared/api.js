...
export const authApi = {
  ...
  // Always authenticate by USERNAME
  loginEphemeral(username, password) {
    const body = new URLSearchParams({ username, password });
    return _request("/auth/login", {
      method: "POST",
      body,
      auth: false,   // do NOT send Authorization header
      form: true     // tell _request/jreq not to JSON-encode
    });
  },
};
/* -------------------------------------------------------
...
export const inquiriesApi = {
  ...
  async respond(inquiryId, responseText) {
    return _request(`/inquiries/${inquiryId}/respond`, {
      method: "PATCH",
      body: { response: responseText }   // <-- body is only {response}
    });
  }
};
/* -------------------------------------------------------
